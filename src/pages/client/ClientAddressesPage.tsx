  ...
  return (
    <DashboardLayout type="client" title={t("sidebar.myAccount")}>
      <div className="flex items-center justify-between mb-6">
        ...
        <Button size="sm"><Plus className="me-1 h-4 w-4" /> {t("common.add")}</Button>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
        {addresses.map(a => (
          <div key={a.id} className={`bg-card rounded-lg border p-4 ${a.default ? 'border-primary' : ''}`}>
            {a.default && <span className="text-xs text-primary font-medium">{t("common.default")}</span>}
            ...
            <div className="flex gap-2 mt-3">
              <Button size="sm" variant="outline"><Edit className="h-3 w-3 me-1" /> {t("common.edit")}</Button>
